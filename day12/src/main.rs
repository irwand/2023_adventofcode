...
fn _count_char(line: &String, character: char) -> usize {
    line.chars().filter(|&c| c == character).count() as usize
}

fn _calc_possibilities(sequence: &String, vec_num: &Vec<u32>) -> u64 {
...
fn main() {
    ...
    for (line_num, line) in lines.iter().enumerate() {
        ...
        let possibilities = _calc_possibilities(&sequence, &vec_num);
        ...
        let possibilities = _calc_possibilities(&sequence_copy, &vec_num);
        println!(
            "{} {} {:?} {}",
